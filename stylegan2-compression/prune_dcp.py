...
def setup_training_loop_kwargs(
    ...
    n_sample   = 5000,  # Total number of samples for estimation: <int>, default = 400
    batch      = 5,     # Size of the batch for estimation: <int>, default = 8
    edit_strength=10.0, # Diversity aware latent perturbation weights: <float>, default = 10.0
    n_direction= 10,    # Number of latent perturbation directions: <int>, default = 10
):
    # ------------------------------------
    ...
    if edit_strength is None:
        edit_strength = 10.0
    assert isinstance(edit_strength, float)
    args.edit_strength = edit_strength
    
    if n_direction is None:
        n_direction = 10
    assert isinstance(n_direction, int)
    args.n_direction = n_direction
...
@click.option('--edit-strength', help='Where to save the output images', type=click.FloatRange(min=0.1, max=20.0), default=10.0)
@click.option('--n-direction', help='Where to save the output images', type=click.IntRange(min=1), default=10)
...
def main(outdir, dataset, gpus, dense, **config_kwarg):
    ...
    print([np.array(grad_score).shape for grad_score in score_list])
    # pruning_score = np.nan_to_num(np.mean(score_array, axis=0))
    pruning_score = np.mean(score_array, axis=0)
    # 
    # print('*'*25)
    # print("Score list")
    # print(score_list)
