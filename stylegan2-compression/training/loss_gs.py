...
class StyleGAN2Loss(Loss):
    def __init__(self, device, G_mapping, G_synthesis, D, G_teacher_mapping, G_teacher_synthesis, augment_pipe=None, style_mixing_prob=0.9, r1_gamma=10, pl_batch_shrink=2, pl_decay=0.01, pl_weight=2, sparse_loss=False, sparse_eta=0.0, kd_loss=True, kd_lambda=3.0):
        super().__init__()
        self.device = device
    ...
    def run_G(self, z, c, sync, both=False):
        ...
        with misc.ddp_sync(self.G_synthesis, sync):
            styles_list = []
            if both and self.return_style_scalars:
                img, styles_list = self.G_synthesis(ws, both and self.return_style_scalars)
            else:
        ...
        return img, ws, img_teacher, ws_teacher, styles_list
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, sync, gain):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, _gen_ws, gen_teacher_img, _gen_teacher_ws, styles_list = self.run_G(gen_z, gen_c, sync=(sync and not do_Gpl), both=True) # May get synced by Gpl.
            ...
            # sparse_loss
            if self.sparse_loss:
                with torch.autograd.profiler.record_function('GAN_Slimming_Sparse oss'):
                    ...
                    loss_Gmain = loss_Gmain + self.sparse_eta * sparse_loss
            ...
            if self.kd_loss:
                with torch.autograd.profiler.record_function('GAN_Slimming_kd_loss'):      
                    ...
                    loss_Gmain = loss_Gmain + self.kd_lambda * kd_percept_loss
