...
def compute_feature_stats_for_dataset(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, data_loader_kwargs=None, max_items=None, **stats_kwargs):
    ...
    # 50k random sampling for datasets
    # from torch.utils.data import DataLoader, RandomSampler, Subset
    # indices = np.random.choice(len(dataset), num_items, replace=False)
    # subset = Subset(dataset, indices)
    # 
    # for images, _labels in torch.utils.data.DataLoader(dataset=subset, batch_size=batch_size, shuffle=False, **data_loader_kwargs):
    #     if images.shape[1] == 1:
    #         images = images.repeat([1, 3, 1, 1])
    #     features = detector(images.to(opts.device), **detector_kwargs)
    #     stats.append_torch(features, num_gpus=opts.num_gpus, rank=opts.rank)
