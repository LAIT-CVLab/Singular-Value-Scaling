...
def setup_training_loop_kwargs(
    ...
    lseg       = None, # Lseg; Language-driven Semantic Segmentation network pickle: <file>
    lseg_prompt= None, # Lseg pre-computed prompt feature: <file>
    n_sample   = 1000, # Total number of samples for estimation: <int>, default = 1000
    batch      = 8,    # Size of the batch for estimation: <int>, default = 8
    noise_prob = 0.05, # Noise Probability for Content-aware pruning: <float>, default = 0.05
):
    # ------------------------------------
    ...
    args.prune_criterion = 'LSeg'
    if lseg is None:
        lseg = 'checkpoints/lseg/lseg.ckpt'
    assert isinstance(lseg, str) and os.path.isfile(lseg)
    assert lseg_prompt is None or  isinstance(lseg_prompt, str) and os.path.isfile(lseg_prompt)
    args.lseg = lseg
    args.lseg_prompt = lseg_prompt
    ...
    if noise_prob is None:
        noise_prob = 0.05
    assert isinstance(noise_prob, float) and 0.0 <= noise_prob <= 1.0
    args.noise_prob = noise_prob    
    
    # Set Network Pruning Ratio
    args.G_kwargs["class_name"] = "training.networks_cagc.Generator"
...
@click.option('--lseg', help='Lseg; Language-driven Semantic Segmentation network pickle', type=str, metavar='PKL')
@click.option('--lseg-prompt', help='Lseg pre-computed prompt feature', type=str, metavar='PT')
...
@click.option('--noise-prob', help='Noise Probability for Content-aware pruning', type=float, metavar='FLOAT')
