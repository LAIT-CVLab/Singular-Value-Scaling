...
def subprocess_fn(rank, args, temp_dir):
    ...
    for metric in args.metrics:
        ...
        result_dict = metric_main.calc_metric(metric=metric, G=G, dataset_kwargs=args.dataset_kwargs,
            num_gpus=args.num_gpus, rank=rank, device=device, progress=progress, cache=args.cache)
        if rank == 0:
...
@click.option('--cache', help='Whether to use pre-computed dataset/generator features [default: True]', type=bool, default=True, metavar='BOOL')

def calc_metrics(ctx, network_pkl, metrics, data, mirror, gpus, verbose, cache):
    """Calculate quality metrics for previous training run or pretrained network pickle.
