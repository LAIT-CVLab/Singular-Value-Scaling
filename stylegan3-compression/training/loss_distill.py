    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, gain, dir_vectors, cur_nimg):
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                gen_img_te = gen_img_te[:batch]
                gen_img_st = gen_img_st[:batch]
                ...
                for layer_name in layer_names:
                    f1 = torch.flatten(feat_s[layer_name], start_dim=1)[:batch].to(torch.float32)
                    f2 = torch.flatten(feat_s[layer_name], start_dim=1)[batch:].to(torch.float32)
                    s_simi = F.cosine_similarity(f1[:,None,:].expand(-1,4,-1), f2[None,:,:].expand(4,-1,-1), dim=2)

                    f1 = torch.flatten(feat_t[layer_name], start_dim=1)[:batch].to(torch.float32)
                    f2 = torch.flatten(feat_t[layer_name], start_dim=1)[batch:].to(torch.float32)
                    t_simi = F.cosine_similarity(f1[:,None,:].expand(-1,4,-1), f2[None,:,:].expand(4,-1,-1), dim=2)
